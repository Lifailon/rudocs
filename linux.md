...
`curl -s https://api.github.com/repos/Lifailon/PS-Commands/contents | sttr json-yaml` конвертировать JSON в YAML
`cat /etc/netplan/*.yaml | sttr yaml-json` конвертировать YAML в JSON
...
`ls /dev/sd*` fdisk -l` отобразить все диски
...
`vgextend vg21 /dev/sdc` vgextend vg21 /dev/sdb` расширить группу vg21 за счет добавленного диска sdc (или vgdisplay)
