...
| [Schedule Parameterized](https://plugins.jenkins.io/parameterized-scheduler)                      | Позволяет определить параметры для запуска Pipeline по расписанию выражения `cron` (`0 21 * * * %env=dev;debug=false`)          |
| [Schedule Build](https://plugins.jenkins.io/schedule-build)                                       | Позволяет запланировать сборку на указанный момент времени для разового выполнения Pipeline                                     |
